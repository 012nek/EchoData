...
(() => {
    ...
    const CONFIG = {
        ...
        BUTTON_CONTAINER_SELECTOR: '[role="group"][aria-label="Message Actions"]',
        CUSTOM_BUTTON_CLASS: 'custom-hover-button',
        BUTTON_PROCESSED_DATA_ATTR: 'data-hover-button-added',
        USERNAME_SELECTOR: 'h3 span.username',
        AVATAR_SELECTOR: 'img.avatar:not(.replyAvatar)',
        MESSAGE_CONTENT_SELECTOR: '[id^="message-content-"]',
        TEXT_COLOR: 'var(--text-normal)',
        HYPERLINK_COLOR: '#00AFF4',
        SVG_ICON: `
            <!-- Info Icon matching Discord's style -->
            <svg viewBox="0 0 24 24" width="24" height="24" class="icon" aria-hidden="true">
            ...
            const observer = new MutationObserver((mutations) => {
                for (const mutation of mutations) {
                    for (const node of mutation.addedNodes) {
                        if (node.nodeType === Node.ELEMENT_NODE && node.matches(CONFIG.MESSAGE_SELECTOR)) {
