...
(function() {
    ...
    const extractUserInfo = (message) => {
        ...
        while (currentMessage && !userId) {
            // Try to extract username if not already found
            if (username === 'Unknown') {
                const usernameElement = currentMessage.querySelector(CONFIG.USERNAME_SELECTOR);
                if (usernameElement) {
            ...
            // If userId is still not found, move to the previous message
            if (!userId) {
    ...
    // Expose the module
    // window.DiscordUI = DiscordUI; // Not exposing to global scope to avoid conflicts
