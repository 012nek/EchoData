...
(function() {
    ...
    // Extract user ID from message avatar or any image containing it
    const extractUserIdFromMessage = (message) => {
        const images = message.querySelectorAll('img[src*="/avatars/"], img[src*="/users/"]');
    ...
    const extractUserInfo = (message) => {
        ...
        // Try to extract from the current message and reply context
        userId = extractUserIdFromMessage(currentMessage) || extractUserIdFromReplyContext(currentMessage);
        username = currentMessage.querySelector(CONFIG.USERNAME_SELECTOR)?.textContent.trim()
            || extractUsernameFromReplyContext(currentMessage)
            || 'Unknown';
        ...
        while (!userId && currentMessage) {
            ...
            if (currentMessage) {
                ...
                if (username === 'Unknown') {
                    username = currentMessage.querySelector(CONFIG.USERNAME_SELECTOR)?.textContent.trim() || 'Unknown';
                }
            }
        }
